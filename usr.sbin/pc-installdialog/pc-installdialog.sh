...
start_edit_menu_loop()
{
  ...
  while :
  do
    dialog --title "${BRAND} Text Install - Edit Menu" --menu "Select:" 18 70 10 disk "Change disk ($SYSDISK)" pool "ZFS pool layout" datasets "ZFS datasets" zpoolcfg "ZFS Pool Config" network "Change networking" view "View install script" edit "Edit install script" back "Back to main menu" 2>/tmp/answer
    if [ $? -ne 0 ] ; then break ; fi
